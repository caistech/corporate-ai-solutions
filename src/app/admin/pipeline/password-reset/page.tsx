...
export default function AdminPasswordResetPage() {
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        setTimeout(() => {
          router.push('/admin/pipeline/login')
        }, 2000)
      }
    } catch (err) {
        ...
        <div className="w-full max-w-sm bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="bg-red-50 border border-red-200 text-red-700 rounded p-4 text-sm">
            ...
            <a
              ...
              className="inline-block px-4 py-2 bg-red-600 text-white font-semibold rounded hover:bg-red-700 transition-colors"
            >
              Back to Admin Login
            </a>
          </div>
      ...
      <div className="min-h-[60vh] flex items-center justify-center px-4 py-8">
        <div className="w-full max-w-sm bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="bg-green-50 border border-green-200 text-green-700 rounded p-4 text-sm">
            ...
            <p>Your password has been reset. Redirecting to admin login…</p>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-sm bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h1 className="text-xl font-bold text-[#0B1F3A] mb-1">Reset Admin Password</h1>
        ...
        <form onSubmit={handleReset} className="space-y-4">
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-3 py-2 pr-10 min-h-[44px] border border-gray-300 rounded text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                placeholder="••••••••"
              />
              <button
                type="button"
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full min-h-[44px] bg-blue-600 text-white font-semibold rounded px-4 py-2 hover:bg-blue-700 disabled:opacity-50 transition-colors"
          >
        ...
        <p className="text-xs text-[#5C6B7A] text-center mt-4">
          ...
          <a href="/admin/pipeline/login" className="text-blue-600 hover:underline">
            Back to admin login
          </a>
        </p>
      </div>
