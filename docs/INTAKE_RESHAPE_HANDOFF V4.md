...
> **STAGED — this handoff is PR1 only.** PR1 = determinism core (~9-10 files — the
> review grew this from an initial "~6 core" estimate; the additions are the harness
> fix, shared validator, the `admit_product` RPC migration, the backfill migration, and
> the Vercel-resolve helper, all real surfaces. Don't let "~6" anchor you into thinking
> something's missing when the count runs higher): one door, coach two-mode, the gate
> (re-derive from the resolved immutable deployment + presence checks + atomic
...
is no-store), so "identical answers → identical verdict" is FALSE across time if the
gate fetches the live alias. A `deployment_id` STAMP alone does NOT pin — it records an
id next to a verdict still computed from whatever the live site serves now (two runs,
same id, post-redeploy → different verdicts → invariant #2 mislabeled). **TRUE PINNING
(Option A):** at admit, (1) resolve the current production deployment for `vercel_project`
via the **Vercel API** (requires a new env var `VERCEL_TOKEN`, alongside `GITHUB_TOKEN`);
(2) derive from THAT deployment's **immutable URL** (`<project>-<hash>.vercel.app`, which
never changes), NOT the mutable alias; (3) stamp the resolved `deployment_id` on the
verdict (`freshness.ts` keys off it). Re-running against the same `deployment_id`
re-fetches the same frozen DOM → same verdict by construction.
**Fallback — no resolvable deployment** (URL-present but no `vercel_project`, or Vercel
resolve fails): the product is NOT deployment-pinned, so it does NOT get a pinned
build-marker gate verdict. Handle it as a no-URL product for the marker checks (no
build-marker gating) — do NOT derive from the live alias and gate on it as if pinned
(that silently reintroduces the non-determinism this fix removes). Either way: never
claim deployment-pinned determinism for an unpinned derivation.
...
reading the SAME helper — not copies:
  ...
  2. the GATE (admit/route.ts) — for a URL-present product the gate RE-DERIVES from
     the **immutable deployment URL it resolves** (Rider 2: Vercel-API resolve →
     `<project>-<hash>.vercel.app`), NOT the mutable live alias and NOT markers passed
     through the client/coach payload. The gate is server-authoritative: client-passed
     gate inputs can be stale or forged (tamperable, LLM-adjacent data in the verdict);
     the live alias drifts on redeploy. Resolving + fetching the frozen deployment makes
     identical `deployment_id` → identical verdict by construction. (Cheap fetches;
     worth the tamper- AND drift-proofing.)
...
- **#10/#11 strip the repurpose residual + the contradiction guard (BLOCKER — do first).**
  The old `new + identifiers → 400` guard at create/route.ts:142-147 was correct under
  the chosen-mode model; under one-door it will **400 the form the moment a URL is typed**
  (the primary deployed-product path). REMOVE/rewrite that guard so a URL-carrying submit
  is accepted and derived as deployed. ALSO resolve the half-removed `repurpose`: either
  fully strip it (the entry_mode CHECK-constraint value + the create/route.ts:184-192
  repurpose branch) OR document it as an explicitly reserved-but-unused slot — do not leave
  it ambiguously half-present (loaded gun). Recommend full strip unless there's a near-term
  use; the pivot/repurpose concept was deferred earlier with no committed design.
- **#9 audit ProductDetailView for a PR1-admitted product.** The post-gate fork is PR2, so
  a PR1-admitted product lands on TODAY's ProductDetailView. Confirm it renders coherently
  WITHOUT the dropped review/repurpose entry_mode state — no blank pane, no read of a field
  that no longer exists. One-line check; prevents an "admitted but blank" surprise.
...
d. Extract the shared identifier-validator (Rider 6); create/route.ts stops validating
   ...
   **REMOVE the new+identifiers→400 guard (create/route.ts:142-147) and resolve the
   repurpose residual (enum value + branch) — findings #10/#11. Without this, the
   one-door form 400s the instant a URL is typed.**
e. The GATE (admit/route.ts): presence checks (existing) + for URL-present products,
   RE-DERIVE markers from the **resolved immutable deployment URL** — resolve current
   prod deployment for `vercel_project` via Vercel API (`VERCEL_TOKEN`), fetch
   `<project>-<hash>.vercel.app`, derive, stamp `deployment_id` (Riders 2, 3) → P2
   reseller fails. **Fallback:** no `vercel_project` / resolve fails → not pinned →
   no build-marker gating (treat as no-URL for markers; don't gate on a live-alias read).
   admit SELECTs the identifiers from product_validation_status (finding #3) before
   writing. On PASS → admit_product RPC writes flip+Z+Y atomically (Rider 4). On FAIL → zero writes.
...
g. AUDIT (finding #9): confirm today's ProductDetailView renders a PR1-admitted product
   coherently — no blank pane, no read of dropped entry_mode/selector state. (Post-gate
   fork is PR2; PR1 cards land on the existing view.)
...
═══ OUTPUT ═══
STEP 0 findings + cherry-pick list first. Then build 0→g. Report files touched, the
...
(2) gate determinism — for URL products the gate resolves+fetches the IMMUTABLE deployment
    URL (not the live alias) and stamps deployment_id; same deployment_id → same verdict
    (test: run twice against a pinned deployment). State it deployment-pinned, NOT "across
    all time"; confirm the unpinned fallback does NOT gate on markers; do NOT claim no-URL
    evidence-teeth. Confirm VERCEL_TOKEN is named as a required env var.
...
2. **Gate determinism — deployment-pinned via immutable fetch** — for URL products the
   gate resolves the prod deployment (Vercel API, `VERCEL_TOKEN`) and derives from the
   immutable `<project>-<hash>.vercel.app` URL, NOT the mutable live alias; same
   `deployment_id` → same verdict. A `deployment_id` *stamp* alone is NOT a pin — confirm
   the gate actually fetches the resolved immutable URL. Unpinned fallback (no
   `vercel_project`) must not gate on markers. If any LLM-judgment reached the verdict,
   that's the regression — it belongs in the coach, not the gate.
3. **One helper, THREE callers** — the coach's live-derive, the **gate's
   server-side re-derive** (admit re-derives from the **immutable `<project>-<hash>.vercel.app`
   deployment URL it resolves via the Vercel API**, NOT the mutable live alias and NOT
   markers passed through the client/coach payload — those are the tamper/flake holes), and
   the post-gate survey all read the same `deriveFieldsFromLiveUrl`. The gate re-deriving
   server-side (not trusting passed markers) + **fetching the resolved immutable deployment**
   (not the alias) is what makes invariant #2 hold — identical deployment + answers →
   identical verdict. (Server-authoritative against forgery; immutable-deployment fetch
   against DOM drift over time; unpinned fallback gets no marker gating at all.)

## Known follow-on (not PR1)
- **Operational dependency — TWO tokens, into feature-preflight BEFORE step e:** PR1's
  gate needs `VERCEL_TOKEN` (Vercel API, to resolve the immutable deployment for pinning)
  AND `GITHUB_TOKEN` (the existing repoExists check). Both are the "works locally, fails
  silently in prod if unset" class — add both to the feature-preflight / env list up front
  (that's the mechanism for surfacing env needs). Without `VERCEL_TOKEN`, URL-present
  products can't be deployment-pinned and fall to the unpinned (no-marker-gate) path.
