...
the gate's marker derivation is pinned to a SPECIFIC, immutable deployment. A
deployment_id STAMP alone is NOT a pin — the survey route stamps an id but still
fetches the mutable live alias, so two runs with the same id after a redeploy can
disagree. TRUE pin mechanism (build this in the gate):
  1. RESOLVE the current production deployment_id for vercel_project via the Vercel
     API (needs a VERCEL_TOKEN env var — a new operational dependency, alongside the
     existing GITHUB_TOKEN). Record it on the verdict.
  2. DERIVE from that deployment's IMMUTABLE URL (`<project>-<hash>.vercel.app`), NOT
     the live alias — so re-running against the same deployment_id re-fetches the SAME
     frozen DOM → same verdict by construction.
  3. FALLBACK: a URL-present product with NO vercel_project can't resolve a deployment
     → derive from the live alias and mark the verdict UNPINNED. Do NOT claim
     deployment-pinned determinism for an unpinned verdict.
Invariant #2 is then true as "identical deployment + answers → identical verdict" —
NOT "identical across all time," and NOT for unpinned (no-vercel_project) verdicts.
State it that way; don't over-claim.
...
reading the SAME helper — not copies:
  ...
  2. the GATE (admit/route.ts) — for a URL-present product the gate RE-DERIVES
     server-side from the IMMUTABLE deployment URL it resolves (Rider 2: resolve the
     prod deployment_id via the Vercel API, fetch `<project>-<hash>.vercel.app`), NOT
     the mutable live alias, and gates on the freshly-derived P1/P2/P3. The gate is
     ...
     determinism invariant). Re-deriving from the pinned deployment makes identical
     deployment + answers → identical verdict by construction. (The helper is a few
     cheap in-process fetches; the cost is negligible and worth the tamper-proofing.)
...
e. The GATE (admit/route.ts): presence checks (existing) + for URL-present products,
   RE-DERIVE markers from the PINNED IMMUTABLE deployment — resolve the prod
   deployment_id for vercel_project via the Vercel API (VERCEL_TOKEN), fetch
   `<project>-<hash>.vercel.app`, derive, stamp the id; fall back to live-alias +
   mark UNPINNED when vercel_project is absent (Riders 2, 3) → P2 reseller fails. admit
...
3. **One helper, THREE callers** — the coach's live-derive, the **gate's
   server-side re-derive** (admit re-derives from the IMMUTABLE deployment URL it
   resolves via the Vercel API, NOT the live alias; it must NOT trust client/coach-passed
   markers — that's the tamper/flake hole), and the post-gate survey all read the same
   `deriveFieldsFromLiveUrl`. The gate re-deriving server-side (not trusting passed
   markers) + **fetching the immutable `<project>-<hash>.vercel.app` deployment** (not
   the mutable alias) is what makes invariant #2 hold — identical deployment + answers →
   identical verdict. (Server-authoritative against forgery; immutable-deployment fetch
   against DOM drift over time; unpinned only when no vercel_project exists.)
